...
export async function getAllCustomers(req, res) {
  ...
  export async function putCustomerBasket  (req, res) {
    try {
      let id = parseInt(req.params.id)
      let newOrder = req.body;
      await customerModel.updateBasket(id, newOrder);
      res.end();
    } catch (error) {
      // res.statusMessage=
      res.status(400).send(error.message);
  ...
  export async function getCustomerBasket (req, res) {
    try {
      ...
      res.json(customer.customerBasket);
    } catch (error) {
      // res.statusMessage=
      res.status(400).send(error.message);
    }
  ...
  export async function deleteItemCustomerBasket (req, res) {
    try {
      let id = parseInt(req.params.id)
      let itemId = parseInt(req.params.itemId)
      await customerModel.removeItemBasket(id,itemId);
      res.end();
    } catch (error) {
      // res.statusMessage=
      res.status(400).send(error.message);
